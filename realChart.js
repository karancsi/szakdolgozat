                ...
                if (index == 0) td.innerHTML = "Cmax";
                    ...
                    for (let i = 0; i < data.length; i++) {
                        if (data[i].EndTimeInInt == max) {
                            ...
                            expcriterion.push(t.getFullYear() + "." + t.getMonth() + "." + t.getDate() + ". " + t.getHours() + ":" + t.getMinutes() + ":" + t.getSeconds() + " ( in munites: " + data[i].EndTimeInInt + ")");
                        }
                    }
                }
                tr.appendChild(td);
            ...
            for (let index = 0; index < uniquejobs.length; index++) {
                ...
                for (let j = 0; j < 2; j++) {
                    ...
                    if (j == 0) {
                        ...
                        var inexp = ((d3.max(tempjobs, function (d) {
                            return d.EndTimeInInt;
                        })) - d3.max(tempjobs, d => d.DueDateTimeInInt));
                        td.innerHTML = inexp;
                        expcriterionL.push(inexp);
                    }
                    tr.appendChild(td);
                }
                tbl.appendChild(tr);
...
function drawWaitBarDiagram() {
    ...
    if (typeof waitChart === 'undefined') {
        ...
        const g = svg.append("g")
            ...
            .data(waitarray)
            .join("rect")
            .attr("x", (d, i) => x(machines[i].Name) + 30)
            .attr("y", (d, i) => y(d))
...
function drawPieDiagram(piechartdata, location) {
    ...
    if (typeof chartPie === 'undefined') {
        ...
        svg
            ...
            .attr('d', arcGenerator)
            .attr('fill', function (d) {
                if (location == "kpiChart") return (color(d.value + Math.random()))
                if (location == "dueChart") {
                    if (d.value == jobintime[0]) return "#de301d";
                    else return "#3de042";
                }
            })
            .attr("stroke", "black")
            ...
            .on("mouseover", function (e, i) {
                ...
                if (location == "kpiChart") {
                    ...
                    tooltipslice.style.background = "lightsteelblue";
                    tooltipslice.style.height = 20;
                    tooltipslice.style.borderRadius = 8;
                    ...
                    tooltipslice.style.display = "block";
        ...
        svg
            ...
            .text(function (d, i) {
                if (location == "kpiChart") {
                    return "M" + machines[i].Id + " " + d.value
                }
                else return d.value + " jobs"
            })
            .attr("transform", function (d) { return "translate(" + arcGenerator.centroid(d) + ")"; })
            .attr("display", function () {
                if (chartdata.length > 13) return "none"
                else return "block"
            })
...
function drawListComponents(diadata) {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        for (let j = 0; j < 2; j++) {
            ...
            //diadata elérés
            expdata.push(diadata.SimulationName);
            expdata.push(diadata.StartDateTime);
            expdata.push(diadata.EndDateTime);
            tr.appendChild(td);
        }
        tbl.appendChild(tr);
    }
...
function exportJson() {
    ...
    //Description
    exportdata.Description.push({ SimulationName: expdata[0], StartDatetime: expdata[1], EndDateTime: expdata[2] });
    //Criterion
    ...
    for (let i = 0; i < machines.length; i++) {
        exportdata.Resources.push({
            ...
            Utilization: utilizationmachine[i],
            OperationNumber: operationsummachine[i],
            Waiting: waitarray[i]
        });
    }
