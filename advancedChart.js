...
function drawDiagram(diadata) {
  ...
  svg.append("g")
    ...
    .attr("y", (d) => y(getMachineIndex(d) - 1) + margin.top + y.bandwidth()+70)
    .attr("dy", "0.35em")
    .attr("dx", -4)
...
function drawDuedateBarChart() {
  ...
  var chartHeight =d3.max(tardinessjob) * 30 + 20;  // Height of chart, from x-axis (ie. y=0)
  var chartWidth = 400;
...
function exportJson(){
 ...
 exportdata.table.push({Cmax: 1, Lmax:machines[0].Id});
